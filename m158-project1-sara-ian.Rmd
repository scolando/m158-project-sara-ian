...
OurData %>%
  ...
  geom_histogram()
```
Distribution of Pulse is approximately unimodal symmetric and bell-shaped.
...
Distribution of Testosterone Levels are skew right with the mode being at 0. 
...
```
The distribution of Physically Active Days is unimodal at approximately 3 days. The data is roughly symmetric but has another peak around 7 days.

```{r fig.cap = "here is the caption", eval=FALSE, echo=FALSE}
OurData %>%
  ggplot(aes(x = Age)) +
  geom_histogram()  
```

```{r fig.cap = "Frequency of Males vs. Females", eval=FALSE, echo=FALSE}
OurData %>%
  ggplot(aes(x = Gender)) + 
  geom_bar() 
```
...
OurData %>%
  ...
  geom_bar()
```
Most common response was "No" with a large percentage of data being N/A (meaning no response).
...
```{r fig.cap = "Frequency for Proportion of Education Completed", echo=FALSE}
OurData %>%
...
OurData %>%
  ...
  geom_bar()
```
The most common response was "Working" and there was also a large percentage of N/A results, where data was not recorded.
...
```{r fig.height=4, fig.width=5, fig.align='center', fig.cap = "Box Plot of (60 second) Pulse vs. Work", echo=FALSE}
glimpse(OurData)
...
There are also many outliers in all three categories of working status according to the box plot.
...
We were surprised by the amount of N/A data for some variables -- especially for those that seemed basic to complete (such as working status).
